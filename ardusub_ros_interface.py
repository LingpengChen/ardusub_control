...
class ArduSubROSInterfaceAdvanced:
    ...
    def setup_button_functions(self):
        ...
        try:
            rospy.loginfo("配置ArduSub按钮功能...")
            
            # 配置灯光控制按钮
            # 按钮10: lights_brighter (功能ID: 32)
            self.set_param('BTN10_FUNCTION', 35, mavutil.mavlink.MAV_PARAM_TYPE_INT8)
            time.sleep(0.1)
            
            # 按钮9: lights_dimmer (功能ID: 33) 
            self.set_param('BTN9_FUNCTION', 36, mavutil.mavlink.MAV_PARAM_TYPE_INT8)
            time.sleep(0.1)
            
            rospy.loginfo("按钮功能配置完成")
            
        except Exception as e:
            rospy.logerr(f"配置按钮功能失败: {e}")
    
    def set_param(self, name, value, param_type, timeout=1):
        """设置参数"""
        if not self.connected or not self.master:
            return False
    ...
    def lights_callback(self, msg):
        """处理灯光控制指令"""
        # 新的控制方式：msg.data > 0 表示变亮，< 0 表示变暗
        if msg.data > 0:
            # 变亮
            self.send_button_event([10])  # 按钮10: lights_brighter
        elif msg.data < 0:
            # 变暗  
            self.send_button_event([9])   # 按钮9: lights_dimmer
    ...
    def set_lights(self, brightness):
        """设置灯光亮度 (0-100) - 使用按钮功能控制"""
        if not self.connected or not self.master:
            rospy.logwarn("ArduSub not connected, cannot set lights")
            return
        
        try:
            # ArduSub灯光控制应该使用按钮功能，而不是直接PWM控制
            # 根据亮度变化决定发送变亮还是变暗按钮
            current_brightness = getattr(self, '_last_brightness', 0)
            
            if brightness > current_brightness:
                # 需要变亮 - 发送lights1_brighter按钮事件
                self.send_button_event([10])  # 按钮10对应lights_brighter
                rospy.loginfo(f"灯光变亮: {brightness}%")
            elif brightness < current_brightness:  
                # 需要变暗 - 发送lights1_dimmer按钮事件
                self.send_button_event([9])   # 按钮9对应lights_dimmer
                rospy.loginfo(f"灯光变暗: {brightness}%")
    ...
    def send_button_event(self, buttons):
        """发送按钮按下事件"""
        if not self.connected or not self.master:
            return
        ...
        try:
            ...
            self.master.mav.manual_control_send(
                self.master.target_system,
                self.control_input['x'],  # x
                self.control_input['y'],  # y
                self.control_input['z'],  # z
                self.control_input['r'],  # r
                button_mask               # buttons
            )
            ...
            self.master.mav.manual_control_send(
                self.master.target_system,
                self.control_input['x'],
                self.control_input['y'],
                self.control_input['z'],
                self.control_input['r'],
                0  # 释放所有按钮
            )
            
        except Exception as e:
            rospy.logerr(f"发送按钮事件失败: {e}")
